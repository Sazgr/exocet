...
struct Move_order {
    ...
    void continuation_update(Move previous, Move current, int bonus) {
        ...
        int index = (previous.piece() * 64 * 12 * 64 + previous.end() * 12 * 64 + current.piece() * 64 + current.end()) * 2;
        continuation[index] += bonus * 1024;
        continuation[index + 1] += std::abs(bonus);
    }

    int continuation_score(Move previous, Move current) {
        if (previous.is_null()) return 0;
        int index = (previous.piece() * 64 * 12 * 64 + previous.end() * 12 * 64 + current.piece() * 64 + current.end()) * 2;
        return continuation[index] / (continuation[index + 1] + 4);
    }
